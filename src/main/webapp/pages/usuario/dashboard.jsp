...
header { background: var(--yellow); padding: 15px 40px; display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; box-shadow:0 3px 6px rgba(0,0,0,0.1); position:sticky; top:0; z-index:1000;}
header h1 { font-weight:900; font-size:2.4rem; color:var(--dark); letter-spacing:2px; margin:0; user-select:none;}
.container { max-width:1000px; margin:40px auto; padding:0 20px; }
.section { background:#fff; border:1px solid var(--yellow); border-radius:var(--radius); padding:25px 30px; margin-bottom:40px; box-shadow:0 5px 20px rgba(255,214,0,0.2);}
h2 { margin:0 0 15px; font-size:1.5rem; color:var(--dark); border-bottom:2px solid var(--yellow); padding-bottom:8px; display:flex; flex-wrap:wrap; justify-content:space-between; align-items:center; gap:8px;}
.edit-button { background:var(--yellow); border:none; padding:8px 16px; border-radius:8px; cursor:pointer; font-weight:bold; color:var(--dark); transition:background 0.2s ease; margin-top:10px;}
.edit-button:hover { background:#ffeb3b; }
.btn-nova-unidade { background:var(--dark); color:var(--yellow); padding:8px 20px; border-radius:30px; font-weight:700; cursor:pointer; border:none; transition:background-color 0.3s ease; user-select:none; font-size:1rem; white-space:nowrap;}
.btn-nova-unidade:hover { background:#000; color:#fff700; }
.units-list { list-style:none; padding:0; margin:0; margin-top:10px; display:flex; flex-direction:column; gap:12px;}
.unit-card { background:#fff; border:1.5px solid var(--yellow); border-radius:var(--radius); padding:16px 20px; display:flex; justify-content:space-between; align-items:center; gap:12px; box-shadow:0 6px 20px rgba(255,214,0,0.2); transition: transform .2s ease, box-shadow .2s ease; }
.unit-card:hover { transform: translateY(-2px); box-shadow:0 10px 30px rgba(255,214,0,0.35);}
.unit-info { display:flex; flex-direction:column; gap:4px; flex:1; min-width:0;}
.unit-title { font-weight:700; font-size:1.1rem; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;}
.unit-sub { font-size:.9rem; color:#555;}
.arrow { flex-shrink:0; font-size:1.5rem; color:var(--dark);}
@media(max-width:900px){ .unit-card{ flex-direction:column; align-items:flex-start;} .arrow{ margin-top:6px;} }
</style>
</head>
<body>
