...
body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #fff8e1; color: #2e2e2e; min-height: 100vh; }
...
h2 { font-size: 1.8rem; margin-bottom: 20px; border-bottom: 2px solid #f7c600; padding-bottom: 8px; }
h3 { font-size: 1.4rem; margin-top: 30px; margin-bottom: 12px; color: #555; }
.card { background: #ffffff; border-radius: 12px; border: 1.5px solid #f7c600; padding: 25px 30px; margin-bottom: 30px; box-shadow: 0 8px 25px rgba(247, 198, 0, 0.25); }
.info { display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; gap: 12px; flex-wrap: wrap; }
.info-label { font-weight: 700; color: #555; flex: 0 0 180px; min-width: 120px; }
.info-value { background: #f9f6d8; padding: 10px 14px; border-radius: 8px; font-size: 1rem; color: #2e2e2e; flex: 1 1 auto; word-wrap: break-word; }
...
.btn { padding: 10px 22px; border: none; border-radius: 30px; font-weight: 700; cursor: pointer; user-select: none; font-size: 0.95rem; display: inline-flex; align-items: center; gap: 6px; transition: background 0.25s ease; text-decoration: none; color: #212121; border: 2px solid #212121; background: transparent; margin-top: 10px; margin-bottom: 30px; }
.btn:hover { background: #212121; color: #ffd600; border-color: #ffd600; text-decoration: none; }
.btn-delete { background: #d32f2f; color: #fff; border-color: #d32f2f; }
.btn-delete:hover { background: #b71c1c; border-color: #b71c1c; color: #fff; }
td .btn { padding: 6px 12px; font-size: 0.9rem; display: inline-block; margin: 3px; border-radius: 20px; border: 1.5px solid #212121; }
table.documentos { width: 100%; border-collapse: collapse; margin-top: 15px; }
table.documentos th, table.documentos td { border: 1px solid #f7c600; padding: 10px 12px; text-align: left; }
table.documentos th { background: #f7c600; color: #212121; font-weight: 700; }
table.documentos tr:nth-child(even) { background: #fff9d1; }
a.arquivo-link { color: #212121; text-decoration: none; font-weight: 700; }
a.arquivo-link:hover { text-decoration: underline; color: #f7c600; }
body.pagina-detalhes-contrato header, body.pagina-detalhes-contrato #header, body.pagina-detalhes-contrato .header { padding-top: 8px !important; padding-bottom: 8px !important; margin-bottom: 20px !important; height: auto !important; line-height: normal !important; }
</style>
</head>
...
<main aria-label="Detalhes do contrato">
    ...
    <% if (usuario != null && usuario.getTipo() == Usuario.TipoUsuario.DONO_GERADORA) { %>
        <% if (contrato.getStatusContrato() == Contrato.StatusContrato.PENDENTE) { %>
            <form action="<%= request.getContextPath() %>/ContratoController" method="post" style="display: inline-block;">
        ...
        <form action="<%= request.getContextPath() %>/pages/contrato/editarContrato.jsp" method="get" style="margin-bottom: 20px;">
            <input type="hidden" name="id" value="<%= contrato.getId() %>" />
            <button type="submit" class="btn">+ Atualizar Contrato</button>
        </form>

        <form action="<%= request.getContextPath() %>/ContratoController" method="post"
              onsubmit="return confirm('Deseja realmente deletar este contrato? Esta ação não pode ser desfeita.');"
              style="margin-bottom: 20px;">
            <input type="hidden" name="action" value="deletar" />
            <input type="hidden" name="id" value="<%= contrato.getId() %>" />
            <button type="submit" class="btn btn-delete">Deletar Contrato</button>
        </form>
    <% } %>
    ...
    <div class="card">
        <% if (listaDocumentos != null && !listaDocumentos.isEmpty()) { %>
            <table class="documentos">
                <thead>
                    <tr>
                        <th>ID</th><th>Tipo</th><th>Descrição</th><th>Data</th><th>Arquivo</th><th>Ações</th>
                    </tr>
                </thead>
                <tbody>
    ...
    <div class="card">
        <% if (listaHistoricos != null && !listaHistoricos.isEmpty()) { %>
            <table class="documentos">
                <thead>
                    <tr>
                        <th>ID</th><th>Data</th><th>Título</th><th>Tipo</th><th>Descrição</th><th>Ações</th>
                    </tr>
                </thead>
                <tbody>
