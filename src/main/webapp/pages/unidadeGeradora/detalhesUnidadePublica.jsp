...
<main aria-label="Detalhes da unidade geradora">
    <a href="<%= request.getContextPath() %>/pages/unidadeGeradora/listaUnidadesDisponiveis.jsp" class="btn btn-back">&larr; Voltar ao Marketplace</a>
    ...
    <div class="card">
        <div class="flex">
            ...
            <div class="info" style="flex: 0 0 200px;">
                <% if (podeEditar) { %>
                    <div class="actions">
                        <form action="<%= request.getContextPath() %>/pages/unidadeGeradora/editarUnidade.jsp" method="get" style="width: 100%;">
                            <input type="hidden" name="id" value="<%= unidade.getId() %>" />
                            <button type="submit" class="btn btn-edit">Editar Unidade</button>
                        </form>

                        <form action="<%= request.getContextPath() %>/UnidadeGeradoraController" method="post" style="width: 100%;" onsubmit="return confirm('Confirma exclusão desta unidade?');">
...
<% if (podeEditar) { %>
    <button id="btnToggleForm" class="btn btn-edit" style="margin-bottom: 20px;">
        + Adicionar Medição
    </button>

    <div id="formMedicao" class="card" style="display:none; max-width: 480px; margin-bottom: 30px;">
        ...
        <form action="<%= request.getContextPath() %>/MedicaoController" method="post">
            ...
            <label for="dataMedicao">Data e Hora da Medição:</label><br/>
            <input type="datetime-local" id="dataMedicao" name="dataMedicao" required style="width:100%; margin-bottom:15px; padding:8px;" />

            <label for="energiaGerada">Energia Gerada (kWh):</label><br/>
            <input type="number" step="0.01" min="0" id="energiaGerada" name="energiaGerada" required style="width:100%; margin-bottom:15px; padding:8px;" />

            <label for="energiaConsumidaLocalmente">Energia Consumida Localmente (kWh):</label><br/>
            <input type="number" step="0.01" min="0" id="energiaConsumidaLocalmente" name="energiaConsumidaLocalmente" required style="width:100%; margin-bottom:15px; padding:8px;" />

            <label for="energiaInjetadaNaRede">Energia Injetada na Rede (kWh):</label><br/>
            <input type="number" step="0.01" min="0" id="energiaInjetadaNaRede" name="energiaInjetadaNaRede" required style="width:100%; margin-bottom:15px; padding:8px;" />

            <button type="submit" class="btn btn-edit" style="width: 100%;">Adicionar Medição</button>
        </form>
    </div>
...
<% } %>
    ...
    <% if (!unidade.getCpfCnpjUsuario().equals(usuarioDetalhesUnidade.getCpfCnpj())) { %>
        <form action="<%= request.getContextPath() %>/pages/contrato/cadastrarContrato.jsp" method="get" style="text-align:center; margin-top: 30px;">
            <input type="hidden" name="unidadeGeradoraId" value="<%= unidade.getId() %>" />
            <button type="submit" class="btn btn-contract">Firmar Contrato</button>
        </form>
    <% } %>
