...
<main aria-label="Detalhes da unidade geradora">
    ...
    <button type="submit" class="btn btn-edit">Editar Unidade</button>
                        ...
                        <form action="<%= request.getContextPath() %>/UnidadeGeradoraController" method="post" style="display:inline;" onsubmit="return confirm('Confirma exclusão desta unidade?');">
...
<% if (podeEditar) { %>
    ...
    <div id="formMedicao" class="card" style="display:none; max-width: 480px; margin-bottom: 30px;">
        ...
        <form action="<%= request.getContextPath() %>/MedicaoController" method="post">
            ...
            <input type="datetime-local" id="dataMedicao" name="dataMedicao" value="<%=java.time.LocalDateTime.now().withSecond(0).withNano(0).toString()%>" placeholder="Selecione a data e hora da medição" required style="width: 100%; margin-bottom: 15px; padding: 8px;" />
